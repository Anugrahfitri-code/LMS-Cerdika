                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('courses.update', $course) }}" method="POST">
                        ...
                        <div>
                            ...
                            <input id="title" name="title" type="text" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('title', $course->title) }}" required>
                        </div>

                        <div class="mt-4">
                            <label for="description" class="block font-medium text-sm text-gray-700">Deskripsi</label>
                            <textarea id="description" name="description" rows="4" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">{{ old('description', $course->description) }}</textarea>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <input id="start_date" name="start_date" type="date" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('start_date', $course->start_date) }}" required>
                        </div>

                        <div class="mt-4">
                            <label for="end_date" class="block font-medium text-sm text-gray-700">Tanggal Selesai</label>
                            <input id="end_date" name="end_date" type="date" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('end_date', $course->end_date) }}" required>
                        </div>
                        ...
                        <div class="mt-4">
                            ...
                            <select id="is_active" name="is_active" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                                <option value="1" @selected(old('is_active', $course->is_active) == 1)>Aktif</option>
                                <option value="0" @selected(old('is_active', $course->is_active) == 0)>Non-Aktif</option>
                            </select>
                        </div>
